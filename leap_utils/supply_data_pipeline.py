...
from leap_utils.config import scenario_dict
...
from leap_utils.all_products_and_flows import ESTO_PRODUCT_LIST
...
ESTO_DATA_PATH = DATA_DIR / "merged_file_energy_ALL_20250814.csv"
...
def normalize_supply_flow_total(flow_key, total_value):
    ...
    try:
        if flow_key in OUTPUT_FLOW_KEYS:
...
SUPPLY_MEASURES = [
    {"name": "Imports", "flow_key": "imports", "units": "Gigajoule", "per": ""},
    {"name": "Exports", "flow_key": "exports", "units": "Gigajoule", "per": ""},
    {
...
DEFAULT_EXPORT_OUTPUT_DIR = os.path.join("outputs", "leap_exports")
EXPORT_OUTPUT_DIR = os.environ.get("SUPPLY_LEAP_EXPORT_DIR", DEFAULT_EXPORT_OUTPUT_DIR)
...
def load_code_to_name_mapping(path_candidates):
    ...
    Outputs:
        Dict mapping code/label to name.
    Side effects:
        Reads from disk.
    """
    try:
...
def map_code_label(label, code_to_name_mapping):
    ...
    Inputs:
        ...
        code_to_name_mapping: Dict of code/label -> name.
    Outputs:
...
#%%
######### WORKFLOW CONTROLS #########
RUN_SUPPLY_ANALYSIS = True
RUN_LIST_FUELS = True
INCLUDE_ALL_ECONOMIES = True
ALL_ECONOMY_LABEL = "ALL"
...
EXPORT_DATASET_KEY = "esto"
EXPORT_DIR = Path(REPO_ROOT) / "outputs" / "leap_exports"
EXPORT_FILE_NAME = "supply_leap_imports_ALL_CurrentAccounts_Reference_Target.xlsx"
SCENARIO_TO_RUN = "Target"
FILL_BRANCHES_FROM_EXPORT_FILE = True
HANDLE_CURRENT_ACCOUNTS_TOO = True
RUN_SUPPLY_LEAP_IMPORT = False
SHEET_NAME = "LEAP"
#%%


def prepare_supply_assets(
    include_all_economies: bool = INCLUDE_ALL_ECONOMIES,
    ...
    if include_all_economies:
        ninth_data = add_all_economy_total(
            ninth_data, ninth_year_cols, ALL_ECONOMY_LABEL
        )
        esto_data = add_all_economy_total(
            esto_data, esto_year_cols, ALL_ECONOMY_LABEL
        )
...
def run_supply_pipeline(
    ...
    if run_list_fuels or run_supply_analysis:
        assets = prepare_supply_assets(include_all_economies=include_all_economies)
