...
>>> export_df = pd.read_excel("../outputs/USA_transport_leap_export_Target.xlsx")
    ...
    for esto_key, rules in branch_mapping_rules.items():
        ...
        for rule in rules:
            try:
                ...
                branch_path = build_branch_path(rule["branch_tuple"], root=rule.get("root", "Demand"))
                raise RuntimeError(
                    f"Failed to calculate LEAP energy for ESTO key {esto_key} at branch {branch_path}."
                ) from exc
            leap_total += energy
        ...
        if abs(leap_total - esto_total) > tolerance and scale_factor != 1.0 and finite_scale_factor:
            for rule in rules:
                try:
                    ...
                    branch_path = build_branch_path(rule["branch_tuple"], root=rule.get("root", "Demand"))
                    raise RuntimeError(
                        f"Failed to apply reconciliation adjustment for ESTO key {esto_key} at branch {branch_path} "
                        f"using scale factor {scale_factor}."
                    ) from exc
