...
def _normalize_export_column_name(value) -> str:
    """Normalize LEAP header cells into stable string column names."""
...
def read_export_sheet(path, sheet_name="Export"):
    ...
    return header_rows, data, columns


def write_export_sheet(path, sheet_name, header_rows, columns, data):
    """Write a LEAP export sheet using preserved header rows and column order."""
    header_rows = header_rows.copy()
    header_rows.columns = columns
    header_row_df = pd.DataFrame([columns], columns=columns)
    output = pd.concat([header_rows, header_row_df, data], ignore_index=True)
    path = Path(path)
