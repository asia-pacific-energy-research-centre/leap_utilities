...
OUTPUT_DIR = REPO_ROOT / "outputs" / "aperc_reference_aggregation_prototype"
...
def _build_aggregated_total(total_long: pd.DataFrame) -> pd.DataFrame:
    if total_long.empty:
    ...
    for year in sorted(total_long["year"].dropna().astype(int).unique().tolist()):
        ...
        value = pd.to_numeric(year_rows["value"], errors="coerce").mean() if contributors else pd.NA
        row = {
            "year": int(year),
...
def _build_aggregated_fuel(fuel_long: pd.DataFrame) -> pd.DataFrame:
    if fuel_long.empty:
        return pd.DataFrame(
    ...
    for fuel in TARGET_FUELS:
        ...
        for year in sorted(f["year"].dropna().astype(int).unique().tolist()):
            ...
            val = pd.to_numeric(fy["value"], errors="coerce").mean() if contributors else pd.NA
            row = {
                "fuel": fuel,
                "year": int(year),
...
def _write_assumptions_md(path: Path, used_optional_9th: bool) -> None:
...
def run_workflow(config: dict[str, Any] | None = None) -> dict[str, str]:
    ...
    if not tpes_fuel_h.empty:
        tpes_fuel_agg = _build_aggregated_fuel(tpes_fuel_h)
    ...
    total_agg = _build_aggregated_total(total_long)
    tpes_agg = _build_aggregated_total(tpes_long)
    fuel_agg = _build_aggregated_fuel(fuel_h)
    ...
    # Write outputs
    source_long_path = output_dir / "source_series_long.csv"
    ...
    notes_path = output_dir / "data_quality_notes.csv"
    assumptions_path = output_dir / "README_assumptions.md"
    ...
    _write_assumptions_md(assumptions_path, used_optional_9th=optional_9th is not None)
